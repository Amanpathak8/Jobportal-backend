...
const createJob = async (req, res) => {
  try {
    const { title, location, description, requiredSkills, requiredEducation, github, linkedin, portfolio, jobType, workMode,
      salary } = req.body;
...
const searchJobs = async (req, res) => {
  try {
    ...
    // 📊 Experience filter (exact or range)
    if (experience) {
...
const savedJobs = async (req, res) => {
  try {
    ...
    const existing = await Saved.findOne({ studentId, jobId });
    ...
    // 👉 SAVE
    await Saved.create({ studentId, jobId });
...
const unsaveJobs = async (req, res) => {
...
const getSavedJobs = async (req, res) => {
  const savedJobs = await Saved.find({ studentId: req.student.id }).populate("jobId");
...
const matchApplication = async (req, res) => {
  try {
    ...
    const requiredLocation = job.location;
    ...
    const normalizedEducation = requiredEducation
      .toLowerCase()
      .replace(/[^a-z0-9]/g, "");
    ...
    const students = await Student.aggregate([
      ...
      // 🎯 SCORE SYSTEM
      {
        $addFields: {
          score: {
            $add: [
              { $multiply: ["$matchedSkillsCount", 15] },
              { $multiply: ["$educationMatch", 5] },
